...
namespace System.Web.Mvc
{
    ...
    {
        public static string PagerBar<T>(this HtmlHelper html, PagedList<T> dataSource) where T : class
        {
        ...
        private static void SetDefaultValue(PagerBarSettings settings)
        {
            ...
            {
                settings.barStyle = BarStyle.viciao;
            }
        ...
        private static string GetPageLink(int linkPageIndex, int currentPageIndex, string text, string currentPageWordFormat, string onclick, string url, string barMark, bool inInframe)
        {
            ...
            url = (!string.IsNullOrEmpty(url)) ? HttpContext.Current.Server.UrlDecode(url).Replace(currentPageWordFormat, linkPageIndex.ToString()) : "";
            ...
            {
                linkHTML = "<span class=\"current\">{0}</span>".With(formatedText);
            }
            else
            {
                linkHTML = "<a " + href + onclick + " {0}>{1}</a>".With(inInframe ? "data-iniframe=\"1\"" : null, text);
            }
            return linkHTML;
        }
    }
